    ...
    <div class="p-6">
      ...
      <div id="tab-dashboard" class="tab-content">
        ...
        <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <div class="glass rounded-xl p-5">
            <p class="text-xs text-gray-500 uppercase tracking-wider mb-1">Journal Posts</p>
            <p class="text-3xl text-white font-semibold"><?= $stats['total_journal'] ?? 0 ?></p>
          </div>
          <div class="glass rounded-xl p-5">
            <p class="text-xs text-gray-500 uppercase tracking-wider mb-1">Subscribers</p>
            <p class="text-3xl text-white font-semibold"><?= $stats['total_subscribers'] ?? 0 ?></p>
          </div>
        </div>
      ...
      <div id="tab-journal" class="tab-content hidden">
        <div class="flex items-center justify-between mb-6">
          <h1 class="text-2xl font-semibold text-white" style="font-family:'Playfair Display',serif">Journal Posts</h1>
          <button onclick="openJournalModal()" class="btn-gold px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2">
        ...
        <div class="glass rounded-xl overflow-hidden">
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b border-yellow-800/20">
                <th class="text-left p-4 text-xs text-gray-500 uppercase tracking-wider">Title</th>
                <th class="text-left p-4 text-xs text-gray-500 uppercase tracking-wider hidden sm:table-cell">Category</th>
                <th class="text-left p-4 text-xs text-gray-500 uppercase tracking-wider hidden md:table-cell">Status</th>
                <th class="text-right p-4 text-xs text-gray-500 uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
      ...
      <div id="tab-pages" class="tab-content hidden">
        <div class="flex items-center justify-between mb-6">
          <h1 class="text-2xl font-semibold text-white" style="font-family:'Playfair Display',serif">Static Pages</h1>
        </div>
        <div class="glass rounded-xl overflow-hidden">
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b border-yellow-800/20">
                <th class="text-left p-4 text-xs text-gray-500 uppercase tracking-wider">Title</th>
                <th class="text-left p-4 text-xs text-gray-500 uppercase tracking-wider hidden sm:table-cell">Slug</th>
                <th class="text-left p-4 text-xs text-gray-500 uppercase tracking-wider hidden md:table-cell">Status</th>
                <th class="text-right p-4 text-xs text-gray-500 uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
      ...
      <div id="tab-faqs" class="tab-content hidden">
        <div class="flex items-center justify-between mb-6">
          <h1 class="text-2xl font-semibold text-white" style="font-family:'Playfair Display',serif">FAQs</h1>
          <button onclick="openFaqModal()" class="btn-gold px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2">
            <i data-lucide="plus" class="w-4 h-4"></i> Add FAQ
          </button>
        </div>
        <div class="space-y-3" id="faqs-list">
          <div class="glass rounded-xl p-8 text-center text-gray-500">Loading...</div>
        </div>
      </div>
      ...
      <div id="tab-subscribers" class="tab-content hidden">
        <div class="flex items-center justify-between mb-6">
          <h1 class="text-2xl font-semibold text-white" style="font-family:'Playfair Display',serif">Newsletter Subscribers</h1>
          <button onclick="exportSubscribers()" class="px-4 py-2 rounded-lg text-sm font-medium border border-yellow-700/40 text-yellow-600 hover:bg-yellow-600/10 transition-colors">
            ↓ Export CSV
          </button>
        </div>
        <div class="glass rounded-xl overflow-hidden">
          <table class="w-full text-sm">
            <thead>
              <tr class="border-b border-yellow-800/20">
                <th class="text-left p-4 text-xs text-gray-500 uppercase tracking-wider">Email</th>
                <th class="text-left p-4 text-xs text-gray-500 uppercase tracking-wider hidden md:table-cell">Subscribed</th>
                <th class="text-right p-4 text-xs text-gray-500 uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
...
<div id="journalModal" class="fixed inset-0 z-50 hidden">
  <div class="absolute inset-0 modal-bg" onclick="closeJournalModal()"></div>
  <div class="absolute inset-0 flex items-start justify-center p-4 pt-6 overflow-y-auto">
    <div class="relative w-full max-w-2xl glass rounded-2xl overflow-hidden mb-8" style="pointer-events:auto">
      <div class="flex items-center justify-between p-5 border-b border-yellow-800/20">
        <h2 id="journalModalTitle" class="text-lg font-semibold text-white" style="font-family:'Playfair Display',serif">New Journal Post</h2>
        <button onclick="closeJournalModal()" class="text-gray-500 hover:text-white"><i data-lucide="x" class="w-5 h-5"></i></button>
      </div>
      <form id="journalForm" class="p-5 space-y-4">
        <input type="hidden" id="journalId" name="id" value="">
        <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
        <div>
          <label class="block text-xs text-gray-400 mb-1.5 uppercase tracking-wider">Title *</label>
          <input type="text" name="title" id="jTitle" required class="w-full px-3 py-2.5 border border-gray-700 rounded-lg text-sm" oninput="autoSlug()">
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-xs text-gray-400 mb-1.5 uppercase tracking-wider">Slug (URL)</label>
            <input type="text" name="slug" id="jSlug" class="w-full px-3 py-2.5 border border-gray-700 rounded-lg text-sm font-mono">
          </div>
          <div>
            <label class="block text-xs text-gray-400 mb-1.5 uppercase tracking-wider">Category</label>
            <input type="text" name="category" id="jCategory" value="Buying Guide" class="w-full px-3 py-2.5 border border-gray-700 rounded-lg text-sm">
          </div>
        </div>
        <div>
          <label class="block text-xs text-gray-400 mb-1.5 uppercase tracking-wider">Cover Image URL</label>
          <input type="url" name="cover_image" id="jCover" placeholder="https://images.unsplash.com/..." class="w-full px-3 py-2.5 border border-gray-700 rounded-lg text-sm">
        </div>
        <div>
          <label class="block text-xs text-gray-400 mb-1.5 uppercase tracking-wider">Excerpt (shown in listings)</label>
          <textarea name="excerpt" id="jExcerpt" rows="2" class="w-full px-3 py-2.5 border border-gray-700 rounded-lg text-sm resize-none"></textarea>
        </div>
        <div>
          <label class="block text-xs text-gray-400 mb-1.5 uppercase tracking-wider">Content (HTML supported)</label>
          <textarea name="content" id="jContent" rows="8" class="w-full px-3 py-2.5 border border-gray-700 rounded-lg text-sm font-mono text-xs resize-y"></textarea>
          <p class="text-[10px] text-gray-600 mt-1">Use &lt;h2&gt;, &lt;p&gt;, &lt;strong&gt;, &lt;ul&gt;&lt;li&gt; tags for formatting.</p>
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-xs text-gray-400 mb-1.5 uppercase tracking-wider">Meta Title</label>
            <input type="text" name="meta_title" id="jMetaTitle" class="w-full px-3 py-2.5 border border-gray-700 rounded-lg text-sm">
          </div>
          <div>
            <label class="block text-xs text-gray-400 mb-1.5 uppercase tracking-wider">Meta Description</label>
            <input type="text" name="meta_description" id="jMetaDesc" class="w-full px-3 py-2.5 border border-gray-700 rounded-lg text-sm">
          </div>
        </div>
        <div class="flex items-center gap-3">
          <label class="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" name="published" id="jPublished" class="w-4 h-4 accent-yellow-600">
            <span class="text-sm text-gray-300">Published (visible to visitors)</span>
          </label>
        </div>
        <div class="flex justify-end gap-3 pt-2">
          <button type="button" onclick="closeJournalModal()" class="px-5 py-2 rounded-lg text-sm border border-gray-700 text-gray-400 hover:text-white transition-colors">Cancel</button>
          <button type="submit" class="btn-gold px-5 py-2 rounded-lg text-sm font-medium">Save Post</button>
        </div>
      </form>
    </div>
  </div>
...
<div id="pageModal" class="fixed inset-0 z-50 hidden">
  <div class="absolute inset-0 modal-bg" onclick="closePageModal()"></div>
  <div class="absolute inset-0 flex items-start justify-center p-4 pt-6 overflow-y-auto">
    <div class="relative w-full max-w-2xl glass rounded-2xl overflow-hidden mb-8" style="pointer-events:auto">
      <div class="flex items-center justify-between p-5 border-b border-yellow-800/20">
        <h2 class="text-lg font-semibold text-white" style="font-family:'Playfair Display',serif">Edit Page</h2>
        <button onclick="closePageModal()" class="text-gray-500 hover:text-white"><i data-lucide="x" class="w-5 h-5"></i></button>
      </div>
      <form id="pageForm" class="p-5 space-y-4">
        <input type="hidden" id="pageId" name="id" value="">
        <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-xs text-gray-400 mb-1.5 uppercase tracking-wider">Title *</label>
            <input type="text" name="title" id="pgTitle" required class="w-full px-3 py-2.5 border border-gray-700 rounded-lg text-sm">
          </div>
          <div>
            <label class="block text-xs text-gray-400 mb-1.5 uppercase tracking-wider">Slug</label>
            <input type="text" name="slug" id="pgSlug" class="w-full px-3 py-2.5 border border-gray-700 rounded-lg text-sm font-mono">
          </div>
        </div>
        <div>
          <label class="block text-xs text-gray-400 mb-1.5 uppercase tracking-wider">Content (HTML)</label>
          <textarea name="content" id="pgContent" rows="10" class="w-full px-3 py-2.5 border border-gray-700 rounded-lg text-sm font-mono text-xs resize-y"></textarea>
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-xs text-gray-400 mb-1.5 uppercase tracking-wider">Meta Title</label>
            <input type="text" name="meta_title" id="pgMetaTitle" class="w-full px-3 py-2.5 border border-gray-700 rounded-lg text-sm">
          </div>
          <div>
            <label class="block text-xs text-gray-400 mb-1.5 uppercase tracking-wider">Meta Description</label>
            <input type="text" name="meta_description" id="pgMetaDesc" class="w-full px-3 py-2.5 border border-gray-700 rounded-lg text-sm">
          </div>
        </div>
        <label class="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" name="published" id="pgPublished" class="w-4 h-4 accent-yellow-600">
          <span class="text-sm text-gray-300">Published</span>
        </label>
        <div class="flex justify-end gap-3 pt-2">
          <button type="button" onclick="closePageModal()" class="px-5 py-2 rounded-lg text-sm border border-gray-700 text-gray-400 hover:text-white">Cancel</button>
          <button type="submit" class="btn-gold px-5 py-2 rounded-lg text-sm font-medium">Save Page</button>
        </div>
      </form>
    </div>
  </div>
...
<div id="faqModal" class="fixed inset-0 z-50 hidden">
  <div class="absolute inset-0 modal-bg" onclick="closeFaqModal()"></div>
  <div class="absolute inset-0 flex items-center justify-center p-4">
    <div class="relative w-full max-w-md glass rounded-2xl overflow-hidden" style="pointer-events:auto">
      <div class="flex items-center justify-between p-5 border-b border-yellow-800/20">
        <h2 id="faqModalTitle" class="text-lg font-semibold text-white" style="font-family:'Playfair Display',serif">Add FAQ</h2>
        <button onclick="closeFaqModal()" class="text-gray-500 hover:text-white"><i data-lucide="x" class="w-5 h-5"></i></button>
      </div>
      <form id="faqForm" class="p-5 space-y-4">
        <input type="hidden" id="faqId" name="id" value="">
        <input type="hidden" name="csrf_token" value="<?= e($csrf) ?>">
        <div>
          <label class="block text-xs text-gray-400 mb-1.5 uppercase tracking-wider">Question *</label>
          <input type="text" name="question" id="fqQuestion" required class="w-full px-3 py-2.5 border border-gray-700 rounded-lg text-sm">
        </div>
        <div>
          <label class="block text-xs text-gray-400 mb-1.5 uppercase tracking-wider">Answer *</label>
          <textarea name="answer" id="fqAnswer" rows="4" required class="w-full px-3 py-2.5 border border-gray-700 rounded-lg text-sm resize-none"></textarea>
        </div>
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-xs text-gray-400 mb-1.5 uppercase tracking-wider">Sort Order</label>
            <input type="number" name="sort_order" id="fqSort" value="0" class="w-full px-3 py-2.5 border border-gray-700 rounded-lg text-sm">
          </div>
          <div class="flex items-end pb-2.5">
            <label class="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" name="active" id="fqActive" checked class="w-4 h-4 accent-yellow-600">
              <span class="text-sm text-gray-300">Active</span>
            </label>
          </div>
        </div>
        <div class="flex justify-end gap-3 pt-2">
          <button type="button" onclick="closeFaqModal()" class="px-5 py-2 rounded-lg text-sm border border-gray-700 text-gray-400">Cancel</button>
          <button type="submit" class="btn-gold px-5 py-2 rounded-lg text-sm font-medium">Save FAQ</button>
        </div>
      </form>
    </div>
  </div>
...
async function loadJournalPosts() {
  const res  = await fetch('/admin/ajax.php?action=get_journal_posts');
  const data = await res.json();
  const tbody = document.getElementById('journal-table');
  if (!data.data || !data.data.length) {
    tbody.innerHTML = '<tr><td colspan="4" class="p-8 text-center text-gray-500">No journal posts yet. Click "New Post" to create one.</td></tr>';
    return;
  }
  tbody.innerHTML = data.data.map(p => `
    <tr class="product-row border-b border-gray-800/50">
      <td class="p-4">
        <div class="flex items-center gap-3">
          ${p.cover_image ? `<div class="w-12 h-8 rounded overflow-hidden flex-shrink-0"><img src="${escHtml(p.cover_image)}" class="w-full h-full object-cover"></div>` : ''}
          <div>
            <p class="text-sm text-white font-medium">${escHtml(p.title)}</p>
            <p class="text-xs text-gray-600 font-mono">/journal/${escHtml(p.slug)}</p>
          </div>
        </div>
      </td>
      <td class="p-4 hidden sm:table-cell text-xs text-gray-400">${escHtml(p.category || '')}</td>
      <td class="p-4 hidden md:table-cell">
        <span class="px-2 py-0.5 rounded-full text-[10px] ${p.published == 1 ? 'bg-green-900/40 text-green-400' : 'bg-gray-800 text-gray-500'}">${p.published == 1 ? 'Published' : 'Draft'}</span>
      </td>
      <td class="p-4 text-right">
        <div class="flex items-center justify-end gap-2">
          <a href="/journal/${escHtml(p.slug)}" target="_blank" class="p-1.5 text-gray-500 hover:text-yellow-400 transition-colors"><i data-lucide="external-link" class="w-4 h-4"></i></a>
          <button onclick="editJournalPost(${p.id})" class="p-1.5 text-gray-500 hover:text-yellow-600 transition-colors"><i data-lucide="pencil" class="w-4 h-4"></i></button>
          <button onclick="deleteJournalPost(${p.id},'${escHtml(p.title)}')" class="p-1.5 text-gray-500 hover:text-red-400 transition-colors"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
        </div>
      </td>
    </tr>
  `).join('');
...
function autoSlug() {
  ...
  if (!document.getElementById('journalId').value) {
    document.getElementById('jSlug').value = title.toLowerCase()
      .replace(/[^a-z0-9\s-]/g,'').replace(/\s+/g,'-').replace(/-+/g,'-').trim();
  }
}

function openJournalModal(postId = null) {
  document.getElementById('journalModal').classList.remove('hidden');
  document.getElementById('journalModalTitle').textContent = postId ? 'Edit Post' : 'New Journal Post';
  document.getElementById('journalForm').reset();
  document.getElementById('journalId').value = postId || '';
  if (!postId) return;
  fetch(`/admin/ajax.php?action=get_journal_post&id=${postId}`)
    .then(r => r.json()).then(d => {
      if (!d.data) return;
      const p = d.data;
      document.getElementById('jTitle').value    = p.title;
      document.getElementById('jSlug').value     = p.slug;
      document.getElementById('jCategory').value = p.category || '';
      document.getElementById('jCover').value    = p.cover_image || '';
      document.getElementById('jExcerpt').value  = p.excerpt || '';
      document.getElementById('jContent').value  = p.content || '';
      document.getElementById('jMetaTitle').value = p.meta_title || '';
      document.getElementById('jMetaDesc').value  = p.meta_description || '';
      document.getElementById('jPublished').checked = p.published == 1;
    });
}

function editJournalPost(id) { openJournalModal(id); }
...
document.getElementById('journalForm').addEventListener('submit', async function(e) {
  e.preventDefault();
  const data = await adminAjaxForm(this, { action: 'save_journal_post' });
  if (data.success) { showToast(data.message); closeJournalModal(); loadJournalPosts(); }
  else showToast(data.error || 'Error saving post', 'error');
});

async function deleteJournalPost(id, title) {
  if (!confirm(`Delete "${title}"?`)) return;
  const data = await adminAjax({ action: 'delete_journal_post', id });
  if (data.success) { showToast('Post deleted'); loadJournalPosts(); }
  else showToast(data.error, 'error');
}

// ---- PAGES ----
async function loadPages() {
  const res  = await fetch('/admin/ajax.php?action=get_pages');
  const data = await res.json();
  const tbody = document.getElementById('pages-table');
  if (!data.data || !data.data.length) {
    tbody.innerHTML = '<tr><td colspan="4" class="p-8 text-center text-gray-500">No pages found. Run /migrate.php first.</td></tr>';
    return;
  }
  tbody.innerHTML = data.data.map(p => `
    <tr class="product-row border-b border-gray-800/50">
      <td class="p-4 text-sm text-white">${escHtml(p.title)}</td>
      <td class="p-4 hidden sm:table-cell text-xs text-gray-400 font-mono">/page/${escHtml(p.slug)}</td>
      <td class="p-4 hidden md:table-cell">
        <span class="px-2 py-0.5 rounded-full text-[10px] ${p.published == 1 ? 'bg-green-900/40 text-green-400' : 'bg-gray-800 text-gray-500'}">${p.published == 1 ? 'Published' : 'Hidden'}</span>
      </td>
      <td class="p-4 text-right">
        <div class="flex items-center justify-end gap-2">
          <a href="/page/${escHtml(p.slug)}" target="_blank" class="p-1.5 text-gray-500 hover:text-yellow-400 transition-colors"><i data-lucide="external-link" class="w-4 h-4"></i></a>
          <button onclick="editPage(${p.id})" class="p-1.5 text-gray-500 hover:text-yellow-600 transition-colors"><i data-lucide="pencil" class="w-4 h-4"></i></button>
        </div>
      </td>
    </tr>
  `).join('');
...
function closePageModal() { document.getElementById('pageModal').classList.add('hidden'); }

function editPage(id) {
  document.getElementById('pageModal').classList.remove('hidden');
  document.getElementById('pageId').value = '';
  fetch(`/admin/ajax.php?action=get_page&id=${id}`)
    .then(r => r.json()).then(d => {
      if (!d.data) return;
      const p = d.data;
      ...
      document.getElementById('pgMetaTitle').value  = p.meta_title || '';
      document.getElementById('pgMetaDesc').value   = p.meta_description || '';
      document.getElementById('pgPublished').checked = p.published == 1;
    });
}

document.getElementById('pageForm').addEventListener('submit', async function(e) {
  e.preventDefault();
  const data = await adminAjaxForm(this, { action: 'save_page' });
  if (data.success) { showToast('Page saved'); closePageModal(); loadPages(); }
  else showToast(data.error || 'Error saving page', 'error');
});

// ---- FAQS ----
async function loadFaqs() {
  const res  = await fetch('/admin/ajax.php?action=get_faqs');
  const data = await res.json();
  const list = document.getElementById('faqs-list');
  if (!data.data || !data.data.length) {
    list.innerHTML = '<div class="glass rounded-xl p-8 text-center text-gray-500">No FAQs yet. Click "Add FAQ" to start.</div>';
    return;
  }
  list.innerHTML = data.data.map(f => `
    <div class="glass rounded-xl p-5 flex items-start justify-between gap-4">
      <div class="flex-1">
        <p class="text-sm text-white font-medium mb-1">${escHtml(f.question)}</p>
        <p class="text-xs text-gray-400 leading-relaxed">${escHtml(f.answer)}</p>
        <div class="flex items-center gap-3 mt-2">
          <span class="text-[10px] text-gray-600">Order: ${f.sort_order}</span>
          <span class="px-1.5 py-0.5 rounded-full text-[10px] ${f.active == 1 ? 'bg-green-900/40 text-green-400' : 'bg-gray-800 text-gray-500'}">${f.active == 1 ? 'Active' : 'Hidden'}</span>
        </div>
      </div>
      <div class="flex gap-2 flex-shrink-0">
        <button onclick="editFaq(${JSON.stringify(f).replace(/"/g,'&quot;')})" class="p-1.5 text-gray-500 hover:text-yellow-600 transition-colors"><i data-lucide="pencil" class="w-4 h-4"></i></button>
        <button onclick="deleteFaq(${f.id})" class="p-1.5 text-gray-500 hover:text-red-400 transition-colors"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
      </div>
    </div>
  `).join('');
  lucide.createIcons();
...
function openFaqModal() {
  ...
  document.getElementById('fqActive').checked = true;
}
...
function editFaq(f) {
  ...
  document.getElementById('fqSort').value     = f.sort_order;
  document.getElementById('fqActive').checked = f.active == 1;
}
...
document.getElementById('faqForm').addEventListener('submit', async function(e) {
  e.preventDefault();
  const data = await adminAjaxForm(this, { action: 'save_faq' });
  if (data.success) { showToast(data.message || 'Saved'); closeFaqModal(); loadFaqs(); }
  else showToast(data.error || 'Error', 'error');
});

async function deleteFaq(id) {
  if (!confirm('Delete this FAQ?')) return;
  const data = await adminAjax({ action: 'delete_faq', id });
  if (data.success) { showToast('FAQ deleted'); loadFaqs(); }
  else showToast(data.error, 'error');
}
...
async function loadSubscribers() {
  const res  = await fetch('/admin/ajax.php?action=get_subscribers');
  const data = await res.json();
  subscriberData = data.data || [];
  const tbody = document.getElementById('subscribers-table');
  if (!subscriberData.length) {
    tbody.innerHTML = '<tr><td colspan="3" class="p-8 text-center text-gray-500">No subscribers yet</td></tr>';
    return;
  }
  tbody.innerHTML = subscriberData.map(s => `
    <tr class="product-row border-b border-gray-800/50">
      <td class="p-4 text-sm text-white">${escHtml(s.email)}</td>
      <td class="p-4 hidden md:table-cell text-xs text-gray-400">${s.subscribed_at ? new Date(s.subscribed_at).toLocaleDateString() : '—'}</td>
      <td class="p-4 text-right">
        <button onclick="deleteSubscriber(${s.id})" class="p-1.5 text-gray-500 hover:text-red-400 transition-colors"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
      </td>
    </tr>
  `).join('');
  lucide.createIcons();
...
async function deleteSubscriber(id) {
  ...
  const data = await adminAjax({ action: 'delete_subscriber', id });
  if (data.success) { showToast('Subscriber removed'); loadSubscribers(); }
  else showToast(data.error, 'error');
}

function exportSubscribers() {
  if (!subscriberData.length) { showToast('No subscribers to export', 'error'); return; }
  const csv   = 'Email,Subscribed At\n' + subscriberData.map(s => `${s.email},${s.subscribed_at || ''}`).join('\n');
