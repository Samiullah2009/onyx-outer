...
class SiteData {
    ...
    public static function rotateTrending(): void {
        ...
        if (!empty($ids)) {
            ...
            try { self::db()->insert('trending_log', ['product_ids' => json_encode($ids)]); } catch (Exception $e) {}
        }
    }
    ...
    // ---- Journal Posts ----
    public static function getJournalPosts(bool $publishedOnly = true, int $limit = 50, int $excludeId = 0): array {
        $where  = $publishedOnly ? 'WHERE published = 1' : 'WHERE 1=1';
        $params = [];
        if ($excludeId > 0) {
            $where .= ' AND id != ?';
            $params[] = $excludeId;
        ...
        return self::db()->fetchAll(
            "SELECT * FROM journal_posts $where ORDER BY created_at DESC $limitSql",
            $params
        );
    }

    public static function getJournalPost(string $slug): ?array {
    ...
    public static function getJournalPostById(int $id): ?array {
        return self::db()->fetchOne("SELECT * FROM journal_posts WHERE id = ?", [$id]);
    }

    public static function saveJournalPost(array $data, ?int $id = null): int {
        if (empty($data['slug'])) {
        ...
        if ($id) {
            self::db()->update('journal_posts', $postData, 'id = ?', [$id]);
            return $id;
        }
        return self::db()->insert('journal_posts', $postData);
    }

    public static function deleteJournalPost(int $id): void {
        self::db()->delete('journal_posts', 'id = ?', [$id]);
    }
    ...
    public static function getAllPages(): array {
        return self::db()->fetchAll("SELECT * FROM pages ORDER BY slug ASC");
    }

    public static function savePage(array $data, ?int $id = null): int {
        $pageData = [
            'slug'             => $data['slug'],
            'title'            => $data['title'],
            'content'          => $data['content'] ?? '',
        ...
        if ($id) {
            self::db()->update('pages', $pageData, 'id = ?', [$id]);
            return $id;
        }
        return self::db()->insert('pages', $pageData);
    }

    public static function deletePage(int $id): void {
        self::db()->delete('pages', 'id = ?', [$id]);
    }

    // ---- FAQs ----
    public static function getFaqs(bool $activeOnly = true): array {
        $where = $activeOnly ? 'WHERE active = 1' : '';
        return self::db()->fetchAll("SELECT * FROM faqs $where ORDER BY sort_order ASC, id ASC");
    }

    public static function saveFaq(array $data, ?int $id = null): int {
        $faqData = [
        ...
        if ($id) {
            self::db()->update('faqs', $faqData, 'id = ?', [$id]);
            return $id;
        }
        return self::db()->insert('faqs', $faqData);
    }

    public static function deleteFaq(int $id): void {
        self::db()->delete('faqs', 'id = ?', [$id]);
    }

    // ---- Subscribers ----
    public static function getSubscribers(): array {
        try {
            return self::db()->fetchAll("SELECT * FROM subscribers ORDER BY subscribed_at DESC");
        } catch (Exception $e) { return []; }
    }

    public static function deleteSubscriber(int $id): void {
        self::db()->delete('subscribers', 'id = ?', [$id]);
    }
    ...
    public static function buildChatContext(): string {
        ...
        $context = "You are the AI concierge for Onyx & Outer, a curated luxury affiliate store. " .
            ...
            "- You can also refer users to the Onyx Journal at /journal for luxury buying guides";
    ...
    public static function getDashboardStats(): array {
        ...
        try {
            $stats['total_journal'] = $db->fetchOne("SELECT COUNT(*) AS c FROM journal_posts WHERE published = 1")['c'] ?? 0;
        } catch (Exception $e) { $stats['total_journal'] = 0; }
        try {
            $stats['total_subscribers'] = $db->fetchOne("SELECT COUNT(*) AS c FROM subscribers")['c'] ?? 0;
        } catch (Exception $e) { $stats['total_subscribers'] = 0; }
        return $stats;
    }
}
